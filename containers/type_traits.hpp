...
//
// 메타함수로, C++20 이전까지 SFINAE(대체 실패는 오류가 아님, Substitution failure is not an error) 상황을 활용하기 위해 사용하는 방법 중 하나다.
// type traits와 함께 사용해서 함수 오버로딩이나 특수화를 할 수 있도록 했다.
//
// 다음과 같이 활용될 수 있다.
// 1) 추가 함수 인수로
// 2) 반환 유형으로
...
struct integral_constant {
  ...
  operator _Type() { return v; } // struct 안에 정의되었으므로 inline된다.
};
...
// 정수에 대해 템플릿 특수화를 해준다.
//
// 아래 특수화 템플릿이 생성되었을 경우, 다음과 같이 초기화된다.
