...
void _Rb_tree_rebalance_for_insert(_Rb_tree_node_base* __x, _Rb_tree_node_base*& __root)
{
  ...
  while (__x != __root && // root에 도착하지 않았다면 체크를 계속한다
    ...
    if (__x->_M_parent == __x->_M_parent->_M_parent->_M_left) {
      _Rb_tree_node_base* __uncle = __x->_M_parent->_M_parent->_M_right;

      // 1-1. 삼촌이 red -> recoloring
      if (__uncle && __uncle->_M_color == _S_rb_tree_red) {
        __x->_M_parent->_M_color = _S_rb_tree_black;
        __uncle->_M_color = _S_rb_tree_black;
    ...
    // 2. 나는 오른쪽, 삼촌은 왼쪽
      ...
      _Rb_tree_node_base* __uncle = __x->_M_parent->_M_parent->_M_left;
                             ...
                             _Rb_tree_node_base*& __rightmost)
{
  ...
  if (__target != __innode) {   
    ...
    if (__target != __innode->_M_right) {
      ...
      if (__x) __x->_M_parent = __target->_M_parent/* __x_parent */;
  ...
  // 1-1. (__target == __innode)
    ...
    __x_parent = __target->_M_parent;
    if (__x) 
      __x->_M_parent = __target->_M_parent;   
    if (__root == __innode)
      __root = __x;
    else 
...
protected:
  size_type _M_node_count; // 요소 개수 저장
  _Compare _M_key_compare; // 함수 (default: less())
...
public:    
  ...
  void swap(_Rb_tree<_Key,_Value,_GetKeyOfValue,_Compare,_Alloc>& __t) {
    std::swap(_M_header, __t._M_header);// 트리 전체
...
template <class _Key, class _Value, class _GetKeyOfValue, class _Compare, class _Alloc>
typename _Rb_tree<_Key,_Value,_GetKeyOfValue,_Compare,_Alloc>::iterator // 반환값
_Rb_tree<_Key,_Value,_GetKeyOfValue,_Compare,_Alloc>
  ::_M_insert(_Base_ptr __x_, _Base_ptr __y_, const _Value& __v)
{
  ...
  if (__y == _M_header || __x != 0 || // (*)요소가 없거나
      _M_key_compare(_GetKeyOfValue()(__v), _S_key(__y))) { // __v < __y
  ...
  ::insert_unique(const _Value& __v)
{
  ...
  if (_M_key_compare(_S_key(__j._M_node), _GetKeyOfValue()( )))
    {return pair<iterator,bool>(_M_insert(__x, __y, __v), true);}
...
_Rb_tree<_Key,_Val,_KoV,_Compare,_Alloc>
  ::_M_copy(
      ...
      _Link_type __x_parent) // __x_parent에 연결하여 반환한다.
{
  // 1. 오른쪽 서브트리를 해결한다.
  // 1-1. top노드 생성 및 연결
  _Link_type __top = _M_clone_node(__x); // 복사 후 서브트리의 맨 위 노드를 먼저 만들고
  __top->_M_parent = __x_parent; // 연결해둔다.
  ...
  // 2. 왼쪽 서브트리로 계속 이동하면서 해결해나간다.
    ...
    while (__x != 0) {
      ...
      _Link_type __sub_top = _M_clone_node(__x);
      __x_parent->_M_left = __sub_top;
      __sub_top->_M_parent = __x_parent;

      // 2-2. 왼쪽 서브트리 복사
      if (__x->_M_right)
        __sub_top->_M_right = _M_copy(_S_right(__x), __sub_top);
      __x_parent = __sub_top;
      __x = _S_left(__x);
    }
  }
  catch(...)
  ...
  ::_M_clear(_Link_type __x)
{
  while (__x != 0) {
    ...
    _Link_type __next_work = _S_left(__x); // __x의 왼쪽 서브트리는 다음에 작업하기 위해 save.
